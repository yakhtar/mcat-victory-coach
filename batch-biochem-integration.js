...
console.log('🧬 MCAT Victory Platform - Biochemistry Integration Batch Generation');
console.log('🎯 Target: 170 Biochemistry Integration Questions (515+ MCAT Score Focus)');
console.log('🔗 Focus: Metabolic Integration, Regulatory Networks, Cell Signaling, Enzyme Systems');

const openaiClient = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
const claudeClient = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });
const generator = new MCATQuestionGenerator(openaiClient, claudeClient);

async function generateBatchBiochemIntegration() {
    try {
        const database = generator.questionDatabase;
        const currentCount = database.metadata.categories.biology?.biochemistry_integration || 0;
        const target = 170;
        const needed = target - currentCount;
        
        console.log(`\n📊 Current Status:`);
        console.log(`Biochemistry Integration Questions: ${currentCount}/${target}`);
        console.log(`Remaining: ${needed} questions`);
        
        if (needed <= 0) {
            console.log('✅ Biochemistry Integration target already achieved!');
            return;
        }
        
        console.log(`\n🚀 Generating ${needed} Biochemistry Integration questions...`);
        ...
        for (let i = 0; i < needed; i += batchSize) {
            ...
            for (let j = 0; j < questionsInBatch; j++) {
                ...
                try {
                    const question = await generator.generateBiologyQuestion('biochemistry_integration', difficulty, type);
                    if (question) {
                        generated++;
            ...
            const newCount = updatedDb.metadata.categories.biology?.biochemistry_integration || 0;
        ...
        const finalCount = finalDb.metadata.categories.biology?.biochemistry_integration || 0;
        
        console.log(`\n🎉 Biochemistry Integration Generation Complete!`);
...
generateBatchBiochemIntegration();