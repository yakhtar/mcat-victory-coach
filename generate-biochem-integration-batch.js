...
async function generateBiochemIntegrationBatch(batchSize = 20) {
    console.log(`🔬 MCAT Victory Platform - Biochemistry Integration Batch Generation (${batchSize} questions)`);
    console.log('===================================================================================');
    
    const generator = new BiologyQuestionGenerator(openai, anthropic);
    
    // Distribution for Biochemistry Integration (170 total target)
    ...
    for (let i = 0; i < batchSize; i++) {
        ...
        try {
            console.log(`\nGenerating question ${i + 1}/${batchSize}: ${difficulty} ${type} biochemistry integration question...`);
    ...
    console.log('\n🎉 BIOCHEMISTRY INTEGRATION BATCH COMPLETE!');
    console.log('============================================');
...
generateBiochemIntegrationBatch(batchSize)
    .then(results => {
        console.log('\n🚀 Biochemistry Integration batch generation completed!');
        process.exit(0);
    })
    .catch(error => {
        console.error('💥 Biochemistry Integration batch generation failed:', error);
        process.exit(1);
    });