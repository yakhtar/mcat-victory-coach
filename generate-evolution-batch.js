...
async function generateEvolutionBatch(batchSize = 20) {
    console.log(`🦕 MCAT Victory Platform - Evolution Batch Generation (${batchSize} questions)`);
    console.log('===================================================================');
    
    const generator = new BiologyQuestionGenerator(openai, anthropic);
    
    // Distribution for Evolution (80 total target - smallest section)
    ...
    for (let i = 0; i < batchSize; i++) {
        ...
        try {
            console.log(`\nGenerating question ${i + 1}/${batchSize}: ${difficulty} ${type} evolution question...`);
...
generateEvolutionBatch(batchSize)
    .then(results => {
        console.log('\n🚀 Evolution batch generation completed!');
        process.exit(0);
    })
    .catch(error => {
        console.error('💥 Evolution batch generation failed:', error);
        process.exit(1);
    });