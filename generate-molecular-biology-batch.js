...
async function generateMolecularBiologyBatch(batchSize = 20) {
    console.log(`🧬 MCAT Victory Platform - Molecular Biology Batch Generation (${batchSize} questions)`);
    console.log('========================================================================');
    
    const generator = new BiologyQuestionGenerator(openai, anthropic);
    
    // Distribution for Molecular Biology (180 total target)
    ...
    for (let i = 0; i < batchSize; i++) {
        ...
        try {
            console.log(`\nGenerating question ${i + 1}/${batchSize}: ${difficulty} ${type} molecular biology question...`);
    ...
    console.log('\n🎉 MOLECULAR BIOLOGY BATCH COMPLETE!');
    console.log('====================================');
...
generateMolecularBiologyBatch(batchSize)
    .then(results => {
        console.log('\n🚀 Molecular Biology batch generation completed!');
        process.exit(0);
    })
    .catch(error => {
        console.error('💥 Molecular Biology batch generation failed:', error);
        process.exit(1);
    });