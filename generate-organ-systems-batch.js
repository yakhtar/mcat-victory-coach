...
async function generateOrganSystemsBatch(batchSize = 25) {
    console.log(`🫀 MCAT Victory Platform - Organ Systems Batch Generation (${batchSize} questions)`);
    console.log('===============================================================================');
    
    const generator = new BiologyQuestionGenerator(openai, anthropic);
    
    // Distribution for Organ Systems (250 total target - largest section)
    ...
    for (let i = 0; i < batchSize; i++) {
        ...
        try {
            console.log(`\nGenerating question ${i + 1}/${batchSize}: ${difficulty} ${type} organ systems question...`);
...
const batchSize = parseInt(process.argv[2]) || 25;
generateOrganSystemsBatch(batchSize)
    .then(results => {
        console.log('\n🚀 Organ Systems batch generation completed!');
        process.exit(0);
    })
    .catch(error => {
        console.error('💥 Organ Systems batch generation failed:', error);
        process.exit(1);
    });